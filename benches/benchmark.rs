...
fn indexlist_head(n: u32) {
    ...
    let mut accum: u64 = 0;
    (1..=n).for_each(|_| {
        accum += list.remove_first().unwrap() as u64;
    });
...
fn vecdeque_head(n: u32) {
    ...
    let mut accum: u64 = 0;
    (1..=n).for_each(|_| {
        accum += vec.pop_front().unwrap() as u64;
    });
    assert_eq!(accum, 52433920);
}
...
fn linkedlist_head(n: u32) {
    ...
    let mut accum: u64 = 0;
    (1..=n).for_each(|_| {
        accum += list.pop_front().unwrap() as u64;
    });
    assert_eq!(accum, 52433920);
}
...
fn criterion_benchmark(c: &mut Criterion) {
    let count = 10 * 1024;
    c.bench_function("indexlist-head", |b| {
        b.iter(|| indexlist_head(black_box(count)))
    });
    c.bench_function("vec-head", |b| b.iter(|| vec_head(black_box(count))));
    c.bench_function("vecdeque-head", |b| {
        b.iter(|| vecdeque_head(black_box(count)))
    });
    c.bench_function("linkedlist-head", |b| {
        b.iter(|| linkedlist_head(black_box(count)))
    });
    c.bench_function("indexlist-body", |b| {
        b.iter(|| indexlist_body(black_box(count)))
    });
    c.bench_function("vec-body", |b| b.iter(|| vec_body(black_box(count))));
    c.bench_function("vecdeque-body", |b| {
        b.iter(|| vecdeque_body(black_box(count)))
    });
    c.bench_function("indexlist-walk", |b| {
        b.iter(|| indexlist_walk(black_box(count)))
    });
    c.bench_function("indexlist-iter", |b| {
        b.iter(|| indexlist_iter(black_box(count)))
    });
    c.bench_function("linkedlist-iter", |b| {
        b.iter(|| linkedlist_iter(black_box(count)))
    });
}
